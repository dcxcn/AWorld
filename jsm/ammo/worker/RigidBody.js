...
Object.assign( RigidBody.prototype, {
	...
	add: function ( o, extra ) {
		...
		if(o.bonePos){
			var pos0 = math.vector3().fromArray( o.bonePos  ).multiplyScalar( root.invScale );
			var form = math.transform();
			form.identity();
			form.setOrigin( pos0 );
			trans.multiply( form );
			form.free();
	...
	applyOption: function ( b, o ) {
		...
		*/
		...
		if( o.trans){
			...
			tr.identity().fromArray( o.trans);
			if(o.mmdPos){
				var offsetTr = math.transform();
				offsetTr.identity().fromArray( o.mmdPos.concat( o.mmdQuat ) );
				tr.multiply(offsetTr);
				offsetTr.free();
			}
			b.setCenterOfMassTransform( tr );			
			b.getMotionState().setWorldTransform( tr );			
			tr.free();
		}
		p1.free();
