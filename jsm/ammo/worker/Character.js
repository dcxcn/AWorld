...
Object.assign( Character.prototype, {
	...
	step: function ( AR, N ) {
		...
		this.heroes.forEach( function ( hero, id ) {
			...
			n = N + ( id * 8 );
			hero.step( AR, n );
			if(root.key.length!=0)hero.move( root.key[id] );
	...
	control: function ( name ) {
		...
		if(root.key)hero.move( root.key );
...
Object.assign( Hero.prototype, {
	...
	step: function ( Ar, n ) {
		...
		//Ar[ n ] = this.speed;
		//Hr[n] = b.onGround ? 1 : 0;
		Ar[ n ] = this.controller.onGround()?1:0;	
	...
	move: function ( key ) {
		...
		if ( key[ 6 ] == 1 ) this.controller.canJump();

		/*
		if ( key[ 6 ] == 1 && this.controller.onGround() ) { //h.canJump() ){
		...
		if(!this.wasJumping && this.controller.onGround()){
			this.verticalVelocity = 0;			
			if( key[6] == 1){			
				this.wasJumping = true;
			}else{
		...
		if(key[6] == 0 && this.wasJumping ){
			this.verticalVelocity += jumpUpSpeed;
		...
		// rotation
		//console.log('this.angleA ==='+this.angle);
		this.angle -= key[ 0 ] * angleInc;
		//console.log('this.angleB ==='+this.angle);
	...
	setAngle: function ( angle ) {
		...
		this.angle = angle;
		//console.log('setAngle this.angle==='+this.angle);
