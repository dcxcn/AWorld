/*global THREE*/
import { geometryInfo,processGeometry, ConvexGeometry } from './Geometry.js';
import { root, map } from './root.js';
import { Tubular } from '../../threex/geometries/Tubular.js';
/**   _   _____ _   _
*    | | |_   _| |_| |
*    | |_ _| | |  _  |
*    |___|_|_| |_| |_|
	...
	step: function ( AR, N ) {
		...
		this.softs.forEach( function ( b ) {
	        ...
	        if ( t === 2 ) { // rope
	            ...
	            p = g.attributes.position.array;//189  63
	            if ( isWithColor ) c = g.attributes.color.array;

	            if ( t === 5 || t === 4 ) { // softTriMesh // softConvex

	               /* var max = g.numVertices; //42
	                var maxi = g.maxi; //63
	                var pPoint = g.pPoint;//numVertices 42
	                var lPoint = g.lPoint;//totalVertices 63
					...
					var volumePositions = g.attributes.position.array;
					var volumeNormals = g.attributes.normal.array;
					var association = g.ammoIndexAssociation;
...
export function softMesh( o, material ) {
	...
	//o.v = g.realVertices;//126
	//o.i = g.realIndices;//240
	//o.ntri = g.numFaces;//80

	o.v = g.ammoVertices;
	o.i = g.ammoIndices;
	o.ntri = g.ammoIndices.length / 3,

	// position and rotation after get geometry info
	g.translate( o.pos[ 0 ], o.pos[ 1 ], o.pos[ 2 ] );
	g.applyMatrix4( root.tmpM.makeRotationFromQuaternion( root.tmpQ.fromArray( o.quat ) ) );
...
export function softCloth( o, material ) {
	...
	//var div = o.div || [ 16, 16 ];
	var size = o.size || [ 4,3,5,5 ];
	var pos = o.pos || [ 0, 0, 0 ];

	//var max = div[ 0 ] * div[ 1 ];
	var clothWidth = size[0];
	var clothHeight = size[1];
	var clothNumSegmentsZ = clothWidth * size[2];
	var clothNumSegmentsY = clothHeight * size[3];
	var div = [clothNumSegmentsZ+1,clothNumSegmentsY+1];	
	var max = div[ 0 ] * div[ 1 ];	
	var g = new THREE.PlaneBufferGeometry( clothWidth, clothHeight, clothNumSegmentsZ , clothNumSegmentsY);
	g.setAttribute( 'color', new THREE.BufferAttribute( new Float32Array( max * 3 ), 3 ) );
	//g.rotateY( - Math.PI90 );
	//g.translate( -size[0]*0.5, 0, -size[1]*0.5 );
	
	//g.rotateY( Math.PI * 0.5 );
	//g.translate(  pos[ 0 ],  pos[ 1 ] + clothHeight * 0.5,  pos[ 2 ] - clothWidth * 0.5 );
