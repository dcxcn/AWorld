...
var TerrainData = function (position,width, height, altitude, source, scale, cols, rows) {
	...
	var methods = {
        ...
        setHeight: function(source, scale, cols, rows) {
            if (source.type == 'hmap') {			
                this._fromArray(this._loadImage(source.data, cols), scale, cols || 1, rows || 1);
            } else if (source.type == 'hdata'){
               this._fromArray(source.data, scale, cols || 1, rows || 1);
            } else {
			   this._fromArray(source.data, scale, cols || 1, rows || 1);
			}
        ...
        getHeightAt: function(x, z) {
			...
			var ox = this._min.x * this.scale.x,
                oz = this._min.z * this.scale.z,
    ...
    for(var i in methods) {
        if (methods.hasOwnProperty(i)) {
            TerrainData.prototype[i] = methods[i];
        }
    }
