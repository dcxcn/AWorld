...
class Terrain extends Mesh {
	...
	addBottom ( o ){
        ...
        this.bottomMesh = new Mesh( geometry, this.material );
	...
	easing  ( wait ) {
        ...
        var r = window.view.getAzimuthal();
	...
	update_hmap (){
		...
		window.WEngine.view.texture({url:_self.heightMapUrl,name:_self.name+'_hmap',onLoad:function(heightTexture){
			...
			//j 循环 记录第几个像素点 +1
			//k 循环 记录 颜色 +3
			for (var i = 0,j=0,k=0, n = pix.length; i < n; i += (4),j++,k+=(3)) {
				var all = pix[i]+pix[i+1]+pix[i+2];
				var c  = _self.height[j] = all/ 3/ 255;	
				//_self.heightData[ j-1 ] = _self.height[j-1];
				//o.hdata[j++] = all/ 3/ 255 * scaleZ;	
				var id = _self.isReverse ? _self.invId[j] : j;
				var result = _self.isAbsolute ? c : c * _self.size[1];
				_self.heightData[ id ] = result;
				var ccY = (c * _self.size[ 1 ]) + _self.deep;
				
				_self.vertices[ k + 1 ] = ccY;
				if( _self.isWater ){

					cc = [ c * _self.colorBase.r, c * _self.colorBase.g, c * _self.colorBase.b ];
				...
				_self.colors[ k ] = cc[0];
				_self.colors[ k + 1 ] = cc[1];
				_self.colors[ k + 2 ] = cc[2];	
			...
			if( _self.isBorder ){
				...
				while(j--){
					n = j*3;
					if(_self.list[j]!==-1){
						h = _self.height[ _self.list[j] ];
						_self.borderVertices[n+1] = (h * _self.size[1]) + _self.deep;//this.data.height;
						var ee = (0.5 + h+0.5);
						ee = ee > 1 ? 1 : ee;
						ee = ee < 0.5 ? 0.5 : ee;
						_self.borderColors[n] = h * _self.colorBase.r//ee;
						_self.borderColors[n+1] = h * _self.colorBase.g//ee*0.5;
						_self.borderColors[n+2] = h * _self.colorBase.b//ee*0.3;

					} else{
						_self.borderColors[n] = _self.colorBase.r//0.5;
						_self.borderColors[n+1] = _self.colorBase.g//0.25;
						_self.borderColors[n+2] = _self.colorBase.b//0.15;
					}
				}

			}
			_self.physicsUpdate( _self.name, _self.heightData,_self);
